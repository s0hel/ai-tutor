...
import type { Skill, Strand } from "@/lib/skills";
              ...
              .map((entry) => {
                ...
                return (
                  <Link
                    ...
                    href={`/learn/math?profile=${profileId}&skill=${entry.skill.slug}`}
                    className="relative flex flex-col gap-2 rounded-2xl bg-white p-4 shadow-sm ring-2 ring-transparent transition hover:ring-kip-purple/30"
                  >
                    {entry.recommended && (
                      <span className="absolute -top-2 -right-2 rounded-full bg-kip-purple px-2 py-0.5 text-xs font-bold text-white shadow">
                        Recommended
                      </span>
                    )}
